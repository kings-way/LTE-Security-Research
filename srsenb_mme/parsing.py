...
class parsing:
        ...
        for i in protocolIEs_list:#go through the list of all IEs (there are already all mandatory IEs)
            ...
            if id == 26:  # NAS-PDU message -> encapsualted communication between UE and MME inside IE
                ...
                if type(msg).__name__ == 'EMMAttachRequest': # 65
                    print("EMMAttachRequest")
                    decoded = msg['EPSID'][1].decode()
                    ident_type = decoded[0]
                    if ident_type == 1:
                        print("imsi is ", decoded[1])
                        parsing.decide_attach(epcServer, decoded[1], enb_ue_id)
                    else:
                    ...
                    print("EMMTrackingAreaUpdateRequest")
                    parsing.send_identityRequest(epcServer, enb_ue_id)

                elif type(msg).__name__ == 'EMMSecProtNASMessage':
                    print("EMMSecProtNASMessage")
                    ...
                    if any(isinstance(item, pycrate_mobile.NAS.EMMAttachRequest) for item in msg): #attach Request
                        print("EMMAttachRequest")
                        print(msg['EMMAttachRequest']['EPSID'][1])
                        decoded = msg['EMMAttachRequest']['EPSID'][1].decode()
                        ident_type = decoded[0]
                        if ident_type == 1:
                            print("imsi is ", decoded[1])
                            parsing.decide_attach(epcServer, decoded[1] ,enb_ue_id)
                    ...
                    if any(isinstance(item, pycrate_mobile.NAS.EMMIdentityResponse) for item in msg):
                        print("EMMIdentityResponse")
                        decoded = msg['EMMIdentityResponse']['EPSID'][1].decode()
                        ident_type = decoded[0]
                        if ident_type == 1:
                            print("imsi is ", decoded[1])
                            parsing.decide_attach(epcServer, decoded[1] ,enb_ue_id)
    ...
    def send_TAUReject(epcServer, enb_ue_id):
        print("send_TAUReject()")
        parsing.send_NAS_PDU_downlink(
            epcServer, parsing.create_NAS_only_TAUReject(epcServer.TAU_reject_reason), enb_ue_id)
    ...
    def send_attachReject(epcServer, enb_ue_id):
        ...
        """
        print("send_attachReject()")
        parsing.send_NAS_PDU_downlink(
            epcServer, parsing.create_NAS_only_attachReject(epcServer.attach_reject_reason), enb_ue_id)

        parsing.do_RRCConnectionRelease(epcServer, enb_ue_id)
    # create S1 encapsulation message

    def send_NAS_PDU_uplink(epcServer, nas_param, enb_ue_id):
        IEs = []
    ...
    def send_NAS_PDU_downlink(epcServer, nas_param, enb_ue_id: int):
    ...
    def do_RRCConnectionRelease(epcServer, enb_ue_id):
        print("RRCConnectionRelease()")
        IES = [
            {'id': 99, 'criticality': 'reject', 'value': ('UE-S1AP-IDs', ('mME-UE-S1AP-ID', enb_ue_id)) },
            {'id': 2, 'criticality': 'ignore', 'value': ('Cause', ('nas', 'normal-release')) },
            #{'id': 2, 'criticality': 'ignore', 'value': ('Cause', ('radioNetwork', 'cs-fallback-triggered')) },    ## no use, as srsenb will always set casue to 'other'
        ]

        val = ('initiatingMessage',
               {
                'procedureCode': 23, # id-UEContextRelease
                'criticality': 'reject',
